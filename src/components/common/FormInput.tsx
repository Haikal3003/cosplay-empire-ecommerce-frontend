...
export default function FormInput({ id, label, type, ...props }: FormInputProps) {
  ...
  return (
    <div className="flex flex-col w-full relative">
      {label && (
        <label htmlFor={id} className="text-sm capitalize font-medium mb-1">
          {label}
        </label>
      )}
      <div className="relative">
        <input id={id} type={inputType} className="text-sm p-2 border border-gray-300 rounded-lg w-full" {...props} />
