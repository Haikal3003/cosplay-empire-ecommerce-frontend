...
export default function FormInput({ id, label, type, labelClassName = '', inputClassName = '', formClassName = '', ...props }: FormInputProps) {
  ...
  return (
    <div className={`flex flex-col w-full relative ${formClassName}`}>
      {label && (
        <label htmlFor={id} className={`capitalize font-medium mb-1 ${labelClassName}`}>
          {label}
        </label>
      )}
      <div className="relative">
        <input id={id} type={inputType} className={` p-2 border border-gray-300 bg-white rounded-lg w-full ${inputClassName}`} {...props} />
        {type === 'password' && (
